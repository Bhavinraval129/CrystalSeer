  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-playfair text-3xl font-bold text-deep-plum">Add New Product</h1>
          <p className="text-deep-plum/60 mt-1">Fill in the details to add a new crystal to the catalog.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className={cardClass}>
          ...
          {imagePreview ? (
            ...
            <div onClick={() => imageInputRef.current?.click()} className="border-2 border-dashed border-soft-purple-300 rounded-2xl p-10 text-center hover:border-primary-purple transition-colors cursor-pointer bg-lavender-mist">
              ...
              <p className="text-sm text-deep-plum/60 font-medium">Click to upload a crystal image</p>
        ...
        <div className={`${cardClass} space-y-5`}>
          ...
          <div>
            ...
            <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Amethyst Spiritual Cluster" className={fieldClass} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" value={form.shortBenefit} onChange={(e) => setForm({ ...form, shortBenefit: e.target.value })} placeholder="e.g. Calms the mind & activates crown chakra" className={fieldClass} />
          </div>
          <div>
            ...
            {sectionsVisibility.description && (
              <textarea rows={4} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Write a detailed product description..." className={`${fieldClass} resize-none`} />
            )}
          </div>
        ...
        <div className={`${cardClass} space-y-4`}>
          ...
          {sectionsVisibility.benefits && benefits.map((b, i) => (
            <div key={i} className="flex gap-2">
              ...
              {benefits.length > 1 && (
                <button type="button" onClick={() => removeListItem(setBenefits, i)} className="p-2 rounded-lg hover:bg-red-50 text-red-400">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
        ...
        <div className={`${cardClass} space-y-4`}>
          ...
          {sectionsVisibility.ingredients && ingredients.map((ing, i) => (
            <div key={i} className="flex gap-2">
              <input type="text" value={ing} onChange={(e) => updateListItem(setIngredients, i, e.target.value)} placeholder={`Crystal / Ingredient ${i + 1}`} className={fieldClass} />
              {ingredients.length > 1 && (
                <button type="button" onClick={() => removeListItem(setIngredients, i)} className="p-2 rounded-lg hover:bg-red-50 text-red-400">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
        ...
        <div className={`${cardClass} space-y-4`}>
          ...
          {specs.map((spec, i) => (
            <div key={i} className="flex gap-2">
              ...
              {specs.length > 1 && (
                <button type="button" onClick={() => setSpecs(specs.filter((_, idx) => idx !== i))} className="p-2 rounded-lg hover:bg-red-50 text-red-400">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
        ...
        <div className={cardClass}>
          ...
          {sectionsVisibility.howToUse && (
            <textarea rows={3} value={form.howToUse} onChange={(e) => setForm({ ...form, howToUse: e.target.value })} placeholder="Describe how to use or activate this crystal..." className={`${fieldClass} resize-none`} />
          )}
        </div>
        ...
        <div className={`${cardClass} space-y-6`}>
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => setCustomSections([...customSections, { id: Math.random().toString(36).substr(2, 9), title: "", content: "", isVisible: true }])}
              className="text-sm text-primary-purple font-medium flex items-center gap-1 hover:underline"
            >
          ...
          {customSections.length === 0 && (
            <p className="text-sm text-deep-plum/50 text-center py-4">No custom sections added yet.</p>
          )}

          {customSections.map((section, idx) => (
            <div key={section.id} className="border border-soft-purple-200 rounded-xl p-5 space-y-4 bg-lavender-mist/50 relative">
              <button type="button" onClick={() => setCustomSections(customSections.filter((_, i) => i !== idx))} className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-red-50 text-red-400">
                <X className="w-4 h-4" />
              </button>
              <div className="flex gap-4 items-start pr-10">
                <div className="flex-1">
                  ...
                  <input
                    type="text"
                    value={section.title}
                    onChange={(e) => setCustomSections(customSections.map((s, i) => i === idx ? { ...s, title: e.target.value } : s))}
                    placeholder="e.g. Chakra Alignment"
        ...
        <div className="flex gap-3 justify-end pb-8">
          <Link href="/admin/products" className="px-6 py-3 rounded-xl border border-soft-purple-200 text-deep-plum font-medium hover:bg-lavender-mist transition-colors">
            Cancel
          </Link>
          <button type="submit" disabled={isUploading} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-deep-plum text-white font-medium hover:bg-primary-purple transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
            {isUploading ? (
